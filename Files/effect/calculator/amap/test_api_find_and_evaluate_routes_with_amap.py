...
def test_api_find_and_evaluate_routes_with_amap():
    """
    测试/api_find_and_evaluate_routes_with_amap接口
    """
    # 测试数据
    test_data = {
        "start_junction": "福建省三明厦钨新能源",
        "end_junction": "福建省平潭跨境电商园",
        "loads_ton": [10, 15, 12],
        "spacings": [3.5, 1.2],
    ...
    # 发送POST请求
    try:
        response = requests.post(f"{BASE_URL}/api_find_and_evaluate_routes_with_amap", json=test_data)
        ...
        if response.status_code == 200:
            ...
            for i, route in enumerate(route_evaluations, 1):
                ...
                if polyline:
                    print(f"    路径标记点数: {len(polyline.split(';'))}")
                    print(f"    路径标记前5个点: {polyline.split(';')[:5]}")
                else:
                    print(f"    路径标记: 无")
                
                if instruction:
                    print(f"    导航指令: {instruction[:100]}..." if len(instruction) > 100 else f"    导航指令: {instruction}")
                else:
                    print(f"    导航指令: 无")
            ...
            # 保存完整结果到JSON文件
            with open('api_find_and_evaluate_routes_with_amap_test_result.json', 'w', encoding='utf-8') as f:
                json.dump(result, f, indent=2, ensure_ascii=False)
            print("\n完整结果已保存到 api_find_and_evaluate_routes_with_amap_test_result.json")
...
if __name__ == "__main__":
    print("开始测试 /api_find_and_evaluate_routes_with_amap 接口...")
    result, response_time = test_api_find_and_evaluate_routes_with_amap()
...
# 10-17 13:30 ： 测试样例：服务器耗时150s+（配置、内存、网络影响） 待优化响应速度
# 10-17 13:30 ： 应用异步+qwen-turbo， 耗时63.693s